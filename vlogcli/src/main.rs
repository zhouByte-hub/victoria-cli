...
#[derive(Parser, Debug)]
#[command(name = "vlogcli", about, version)]
struct VlogCli {
    #[command(subcommand)]
    command: Option<SubCommand>,

    #[arg(short, long, help = "Set VictoriaLogs service address")]
    path: Option<String>,
}
...
async fn init_status_config() -> Result<(), CliError> {
...
async fn vlog_init() -> Result<(), CliError> {
    log::info!("初始化配置文件");
