...
use crate::UNKONW;
use crate::{
    command::status::Status, error::CliError, executor::Runnable, proj_config::ReqwestConfig,
...
impl Import {
    async fn recursive_decompress(
        ...
        if !file_name.ends_with(".tar.gz") {
            if !file_name.ends_with(".json") {
                let message = CliError::VlogCliError("当前导入的内容不是一个合法的导入文件".into());
                return Err(message);
            }
    ...
    async fn import_file(file: &PathBuf, client: &Client, url: &str) -> Result<(), CliError> {
        let file_name = file
            .file_name()
            .and_then(|s| s.to_str())
            .unwrap_or(UNKONW)
    ...
    async fn import_run(&self, config: &ReqwestConfig) -> Result<(), CliError> {
        ...
        let url = Arc::new(format!(
            "{}{}",
            status.victoria_logs_path, config.victoria_logs_import_path
        ));
    ...
    async fn is_duplicate_upload(file_hash: &str) -> Result<bool, CliError> {
    ...
    async fn update_import_record(file_hash: &str, file_name: &str) -> Result<(), CliError> {
...
mod import_test {
    ...
    use crate::{command::import::Import, error::CliError};
    use tokio::time::Duration;

    #[tokio::test]
    async fn read_test() -> Result<(), CliError> {
        let path = PathBuf::from("/Users/zhoujianing/vlogcli/a/2025-08-18T16:00:01Z.tar.gz");
        if !path.exists() {
            return Ok(());
        }

        let import = Import { file: path };
